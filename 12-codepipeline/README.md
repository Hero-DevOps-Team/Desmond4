...
Stelligent engagements seem to use the same tool-set. As a result, there
...
- Add the CodePipeline Pipeline resource itself, encoding
  ...
  - a 'Deploy' stage with an action that executes
    [this CFN template](https://github.com/stelligent/stelligent-nu/blob/master/12-codepipeline/bucket.yaml)
...
- the CodeBuild resources themselves, including buildspecs
