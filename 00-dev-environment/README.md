...
- [Topic 0: Development Environment](#topic-0-development-environment)
  ...
  - [Lesson 0.1: Setting up your development environment](#lesson-01-setting-up-your-development-environment)
    - [Principle 0.1](#principle-01)
      - [Ephemeral Resource Warning!](#ephemeral-resource-warning)
...
will be using Google Drive for document access, AWS access keys for
programmatic access to an AWS account, GitHub code
repositories, and AWS Cloud9 for your development environment. You
will pair with your mentor to complete these labs.

#### Ephemeral Resource Warning

Please be aware that all resources created in the Stelligent labs
account are considered ephemeral. Every Saturday, [aws-account-janitor](https://github.com/stelligent/aws-account-janitor)
will clean up all resources in the account. You should ensure that
all of your work is written in code so that it is easily reproducible.
If you are in the middle of a training module on a Friday, make sure
it is all in code and you can re-launch your stack(s) on the following
Monday morning.
...
Before you can access resources in the AWS account you will need to generate
temporary credentials using your MFA device and STS. You can do this in a few
...
1. Create a new repository from the [Nu repository template](https://github.com/stelligent/stelligent-nu/generate)
1. Select the owner of the repository
1. Name the new private repository based on the username in your email address:
   _user.name-newhire-training_. For example, a user with an email address "tom.jones@stelligent.com"
   will create a new repo named "tom.jones-newhire-training".
...
1. For each topic you work on, create a new branch.
  When you're ready for a code review, commit and push your files to the branch.
  Then submit a `master` pull request, assigning it to your reviewer.
...
the Cloud9 environment to work on the remainder of the Stelligent Nu
...
- Create a pull request from your branch to the master branch within your forked
  repository and add your mentor as a reviewer
